...
def _detect_delimiter(sample_text):
    ...
    Strategy: for each candidate delimiter (tab, comma, semicolon, pipe),
    parse the header line + the next few data lines as if that were the
    delimiter and count the columns each line produces. The CORRECT
    delimiter produces:

      (a) more than one column on the header (single-column files are
          almost never what we want), AND
      (b) consistent column counts across the header and the data lines
          (the wrong delimiter produces wildly varying counts because
          it's matching characters that happen to appear inside fields).

    This handles the proteinGroups.txt edge case where data fields contain
    semicolon-separated lists of accession IDs (e.g. ``Q9X1H7;P38398``).
    The naïve "count all semicolons" heuristic picks semicolon as the
    delimiter because the data sums dwarf the header sum — but parsing
    the file with semicolons collapses every line to 1 column, while
    parsing with tab gives a clean 92 columns per line. The consistency
    score catches this.

    We use ``csv.reader`` (not ``str.count``) for the column count so that
    quoted fields with embedded delimiters (e.g. ``a,"b,c",d``) count as
    3 columns rather than 4.

    Ties are broken in favour of tab (proteinGroups.txt's historical
    default), then comma, then semicolon, then pipe.
    """
    candidates = "\t,;|"
    # Use the first 10 non-blank lines for scoring. We DON'T strip "preamble"
    # lines (the old heuristic) because that filter incorrectly removed
    # title lines but ALSO kept data-only-delimiter rows that confuse the
    # column-count consistency check. The consistency scorer handles
    # preamble lines naturally — they just contribute 1-column outliers
    # that lower the score for whichever delimiter they're missing, which
    # is the same penalty for every candidate so doesn't affect ranking.
    ...
    best = None
    for d in candidates:
        cols_per_line = [_csv_col_count(line, d) for line in lines]
        header_cols = cols_per_line[0]
        if header_cols < 2:
            # This delimiter doesn't split the header into multiple columns
            # — can't be the right one. (proteinGroups.txt's header with
            # ';' as delimiter parses as 1 column → skipped here.)
            continue
        data_cols = cols_per_line[1:] if len(cols_per_line) > 1 else cols_per_line
        # Consistency: fraction of data lines that match the header's
        # column count. The correct delimiter produces consistent counts;
        # the wrong delimiter produces wildly varying ones because it's
        # hitting characters inside fields rather than between them.
        if data_cols:
            matches = sum(1 for c in data_cols if c == header_cols)
            consistency = matches / len(data_cols)
        else:
            consistency = 1.0
        # Tie-breaker prefers tab → comma → semi → pipe, so we negate
        # the candidate index (lower index = higher tie-break score).
        score = (consistency, header_cols, -candidates.index(d))
...
def _csv_col_count(line, delim):
    """Count delimited columns in a single line, respecting CSV-style
    double-quote quoting. Falls back to a simple count+1 on parse error
    (e.g. unclosed quotes) so a malformed line doesn't break detection.
    """
    try:
        import io as _io
        reader = csv.reader(_io.StringIO(line), delimiter=delim, quotechar='"')
        return len(next(reader))
    except (StopIteration, csv.Error):
        # No row or malformed quote — fall back to literal character count
        return line.count(delim) + 1


def _row_non_empty_count(row):
    """Count non-empty cells in a raw row (tuple from openpyxl or list from
    csv.reader). Treats None and whitespace-only strings as empty."""
    n = 0
    for c in row:
        if c is None:
            continue
...
def _pick_header_row(preview_rows, max_scan=5):
    """Find the row most likely to be the header in a small preview window.

    Many Excel exports prefix the data with a single-cell title row (e.g.
    ``ORIGINAL — counts and stats imported verbatim from ...``); naively
    treating row 0 as headers then yields a 1-column "table" with the rest
    looking blank. Scan the first ``max_scan`` rows for the one with the
    most non-empty cells; the title row will lose to the proper header
    row that has all the column names. Returns the index (0-based) into
    ``preview_rows`` of the chosen header row, or 0 if everything looks
    sparse / equal.
    """
    if not preview_rows:
        return 0
    # Find the maximum non-empty cell count in the scan window
    counts = [_row_non_empty_count(r) for r in preview_rows[:max_scan]]
    max_count = max(counts) if counts else 0
    if max_count <= 1:
        return 0  # Can't distinguish — first row wins
    # Pick the FIRST row whose count is >= 70% of the max. The 70% threshold
    # tolerates a header row that's slightly sparser than later data rows
    # (a trailing optional column that's blank in the header but has some
    # data values further down — uncommon but possible).
    threshold = max(2, int(max_count * 0.7))
...
def _read_xlsx(filepath):
    ...
    Title rows (a one-cell description above the actual table) are
    detected and skipped — the parser scans the first 5 rows for the one
    with the most non-empty cells and treats THAT as the header row.
    Without this, a workbook with a "ORIGINAL — counts imported from..."
    row at the top would be parsed as a 1-column table and fail
    column detection.
    """
    try:
    ...
    try:
        ...
        # Stash all sheet names on the function (read by callers via
        # ``_read_xlsx.last_sheet_names``) so the parsers can surface a
        # "this workbook has 3 sheets, using the first" hint to the user.
        # Module-level state is ugly but the alternative is changing the
        # return signature of _read_table for one rare case — not worth it.
        _read_xlsx.last_sheet_names = list(wb.sheetnames)
        _read_xlsx.last_active_sheet = wb.sheetnames[0]
        # Materialise the whole sheet so we can pick the header row before
        # building dicts. openpyxl's read_only mode keeps memory bounded
        # at ~50 MB even for large sheets.
        all_rows = list(ws.iter_rows(values_only=True))
        if not all_rows:
            return [], [], []
        header_idx = _pick_header_row(all_rows, max_scan=5)
        header_row = all_rows[header_idx]
        # Strip trailing all-None columns (Excel pads to a wider rectangle)
...
def _read_delimited(filepath):
    ...
    Same title-row detection as the xlsx path: the first 5 rows are
    scanned and the one with the most non-empty cells becomes the header
    row. So a CSV that starts with ``# Generated by edgeR\\n`` or a
    similar single-cell preamble is parsed correctly without the user
    ...
    if not all_rows:
        return [], [], []
    header_idx = _pick_header_row(all_rows, max_scan=5)
    headers = [(c or "").strip() for c in all_rows[header_idx]]
    # Strip trailing empty columns (matches xlsx behaviour)
    while headers and headers[-1] == "":
        headers.pop()
    n_cols = len(headers)
    if n_cols == 0:
    ...
    raw_rows = []
    for row in all_rows[header_idx + 1:]:
        # Skip fully-blank lines (csv reader emits [] for empty lines)
...
def _read_table(filepath):
    ...
    try:
        ...
        pass
    # Reset the per-read xlsx metadata so callers don't pick up stale
    # sheet names from a previous upload.
    _read_xlsx.last_sheet_names = []
    _read_xlsx.last_active_sheet = None
    if ext in (".xlsx", ".xlsm"):
...
# Initialise the module-level fields so ``getattr`` calls don't blow up
# before the first xlsx upload.
_read_xlsx.last_sheet_names = []
_read_xlsx.last_active_sheet = None
...
def parse_protein_groups(filepath):
    ...
    return {
        ...
        "format_info": {
            ...
            # If the source was an .xlsx with multiple sheets, surface the
            # full sheet list so the frontend can show "this workbook has N
            # sheets, using 'X'" — a hint to re-export with a different
            # sheet first if the user wanted a different one.
            "xlsx_sheet_names": list(getattr(_read_xlsx, "last_sheet_names", []) or []),
            "xlsx_active_sheet": getattr(_read_xlsx, "last_active_sheet", None),
        },
...
def parse_transcriptomics(filepath):
    ...
    return {
        ...
        "format_info": {
            ...
            # If the source was an .xlsx with multiple sheets, surface the
            # full sheet list so the frontend can show "this workbook has 3
            # sheets, using 'Sheet1'" — a hint to re-export with a different
            # sheet first if the user wanted one of the others.
            "xlsx_sheet_names": list(getattr(_read_xlsx, "last_sheet_names", []) or []),
            "xlsx_active_sheet": getattr(_read_xlsx, "last_active_sheet", None),
