...
def _detect_delimiter(sample_text):
    ...
    Strategy (in priority order):
      ...
      1. Strip any leading "title" lines that contain none of the candidate
         delimiters. These confuse csv.Sniffer because the inconsistent
         column count between the title and the real rows makes Sniffer
         raise. The actual data structure is in the lines that DO have
         delimiters, so feed Sniffer just those.
      2. Run csv.Sniffer on the cleaned sample — handles edge cases like
         quoted fields containing the delimiter.
      3. Fall back to a count-based heuristic: pick the delimiter with the
         highest total occurrence across non-title lines, breaking ties in
         favour of tab (proteinGroups.txt's historical default), then
         comma, semicolon, pipe.
    """
    candidates = "\t,;|"
    # Strip lines that are clearly preamble (no candidate delimiter at all).
    # This is what makes Sniffer choke on files like:
    #   # Generated by edgeR v3.42.4
    #   Locustag,Gene,...
    #   gene1,GENEA,...
    # Sniffer compares the first few lines and bails on inconsistent column
    # counts. Removing the preamble line gives Sniffer consistent input.
    lines = sample_text.split("\n")
    data_lines = [l for l in lines if any(d in l for d in candidates)]
    cleaned = "\n".join(data_lines) if data_lines else sample_text

    # Sniffer needs at least one full line — skip it on tiny samples
    if len(cleaned) >= 16:
        try:
            return csv.Sniffer().sniff(cleaned, delimiters=candidates).delimiter
        except csv.Error:
            pass
    # Count-based fallback. Sum delimiter occurrences across all data lines
    # (not just the first) so a title line with 0 occurrences doesn't drag
    # the score down for the actual delimiter.
    scan = data_lines if data_lines else lines
    scan = [l for l in scan if l.strip()][:8]  # First 8 non-blank lines
    counts = []
    for d in candidates:
        total = sum(l.count(d) for l in scan)
        if total > 0:
